...
public class Runner {
	...
	private void acquireLocks(Lock firstLock, Lock secondLock){
		while(true){
			...
			try{
				gotFirstLock = firstLock.tryLock(); //returns true if lock is locked
				gotSecondLock = secondLock.tryLock();
			}
			finally{
				if(gotFirstLock && gotSecondLock){ //if we have locked both locks then return to main method we have been successful
					return;
				}
				if(gotFirstLock){ //if we have first lock we would unlock it so other things can use it
					firstLock.unlock();
				}
				if(gotSecondLock){
					secondLock.unlock();
